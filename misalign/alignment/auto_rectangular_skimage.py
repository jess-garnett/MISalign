...
Can do local alignment(i.e. from initial manual alignment) without scikit-image 
but requires scikit-image for full search alignment with no initial information.
"""
import logging
...
def overlap_process(
        ...
        array_b:np.ndarray,
        offset_ab:tuple[int,int]|np.ndarray,
        process_function:Callable=np.subtract
        )->np.ndarray:
    """
    Calculate a function on overlapping regions of image a and image b given a translational offset.
    ...
    process_function : Callable[[np.ndarray,np.ndarray],np.ndarray]
        A function that accepts two numpy arrays and returns a numpy array or `numpy.subtract` by default.

    Returns
    -------
    process_result : np.ndarray
        Result of applying the process function to the overlapping array regions.
        Example: The difference (a-b) of the overlap between image a and image b.
    """
    ## Find overlap spans
    a_spans,b_spans=overlap_spans(tuple(offset_ab),array_a.shape,array_b.shape)
    ## Extract overlap regions
    ...
    return process_function(overlap_a,overlap_b)
...
class LocateMetric():
    ...
    @staticmethod
    def norm_mean_squared_difference(overlap_a:np.ndarray,overlap_b:np.ndarray)->float:
        """
        Calculates the mean of the squared difference of two normalized arrays. Arrays are normalized by subtracting their mean and dividing by their standard deviation.

        This locating metric is more resistant to false-positives in low-features areas than most of the other simple locating metrics.
        ...
        return LocateMetric.mean_squared_difference(
                ((overlap_a-np.mean(overlap_a))/np.std(overlap_a)),((overlap_b-np.mean(overlap_b))/np.std(overlap_b)))
    @staticmethod
    ...
    @staticmethod
    def deviation_difference_over_source(overlap_a:np.ndarray,overlap_b:np.ndarray)->float:
        """
        Calculates the standard deviation of the difference of two arrays divided by the sum of the standard deviations of the arrays.
...
class WeightMetric():
    ...
    Weight metrics are intended to be combined with locating metrics to counteract false-positives for full search.
    """
    @staticmethod
...
"""
Prediction & Interpolation Functions
...
class PredictSkimage():
    """
    Group of functions which take two arrays and an offset and predicts an aligned offset using scikit-image functions.
    """
    @staticmethod
...
class RectangularRegistrationResultCompositePredictLocal(RectangularRegistrationResult):
    ...
    def get_plot_setup(self,plot_kwargs:dict|None=None):
        ...
        if plot_predict_sparse not in plot_kwargs:
            plot_kwargs[plot_predict_sparse]=dict()
        
        if "legend_kwargs" not in plot_kwargs[plot_predict_sparse]:
            plot_kwargs[plot_predict_sparse]["legend_kwargs"]=dict(loc="lower left",bbox_to_anchor=(1.3,0))
        if "optimized_offset" not in plot_kwargs[plot_predict_sparse]:
            plot_kwargs[plot_predict_sparse]["optimized_offset"]=self.initial_offset
        return plots,mosaic_rows,plot_kwargs
...
class StrategyFullSearch():
    ...
    def prediction_grid(
            ...
            metric:Callable[[np.ndarray,np.ndarray],float],
            initial_offset:tuple[int,int]|None=None,
            strategy_predict:Callable=PredictSkimage.phase_cross_correlation,
            strategy_edge_avoid=20,
...
def plot_metric_grid(
    ...
    if imshow_kwargs is None:
        imshow_kwargs=dict()
    grid_plot=axs[axs_key].imshow(grid_results,
    ...
    if formatting:
        axs[axs_key].set_xlabel("Rectangular X-offset")
        axs[axs_key].set_ylabel("Rectangular Y-offset")

        divider = axes_grid1.make_axes_locatable(axes=axs[axs_key])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(label="Metric",mappable=grid_plot,cax=cax)
...
def plot_interpolation_grid(
    ...
    plot_metric_grid(
        ...
        grid_results=result.interp_results,
        optimized_offset=optimized_offset,
        reference_offset=reference_offset,
        axs_key="interpolation_grid",
        title=title,
        overlays=overlays,
        formatting=formatting,
...
def plot_process_overlap(
    ...
    if formatting:
        axs["process_overlap"].set_axis_off()
        ...
        #TODO consider keeping this method vs switching to the style used with metric grid with toggle between horizontal and vertical.
        
        aspect_ratio=overlap.shape[1]/overlap.shape[0] # rows/columns
        if aspect_ratio > 1.5:
            match aspect_ratio:
                case ratio if ratio > 20:
                    inset_position = -1
                case ratio if ratio > 10:
                    inset_position = -0.5
                case ratio if ratio > 5:
                    inset_position = -0.2
                case ratio if ratio > 1.5:
                    inset_position = -0.1
            inset_colorbar_kwargs=dict(
                height=0.1,
                width=2,
            ...
            match aspect_ratio:
                case ratio if ratio < 1/20:
                    inset_position = -1.5
                case ratio if ratio < 1/10:
                    inset_position = -1
                case ratio if ratio < 1/5:
                    inset_position = -0.5
                case ratio if ratio < 2/3:
                    inset_position = -0.2
                case ratio if ratio <= 1.5:
                    inset_position = -0.1
            inset_colorbar_kwargs=dict(
        ...
        cax=inset_locator.inset_axes(
            parent_axes=axs["process_overlap"],
...
def plot_result(mis_project:MISProject,relation:MISRelation,axs:dict[str,plt.Axes]|None=None,result:RectangularRegistrationResult|None=None,
    ...
    if result is not None:
        try:
            ...
            added_plots,added_mosaic_rows,plot_kwargs=result.get_plot_setup(plot_kwargs)
            plots.extend(added_plots)
            mosaics.extend(added_mosaic_rows)
...
#TODO potential full search strategy: `gradient_descent`
